...
silver_bullet.prototype.load_bullets = function () {
  ...
  // setTimeout(() => {
  //   // let lg = new LinkGenerator(this.root, slidelist);
  //   let lg = new LinkGenerator(this.root);
  // }, "500");
...
// function getSlide(slidelist, dir) {
function getSlide(dir) {
  ...
  let fileIndex = slidelist.indexOf(file);
  let newFilename = file;
  if (fileIndex >= 0) {
    if (dir === "next") {
      fileIndex++;
      if (slidelist.length <= fileIndex) {
        fileIndex = 0;
      }
    } else {
      fileIndex--;
      if (0 > slidelist.length) {
        fileIndex = 0;
      }   
    }
    newFilename = slidelist(fileIndex);
  }
  return newFilename;
}
...
// function LinkGenerator(root, slidelist) {
function LinkGenerator(root) {
  this.root = root;
  // this.slidelist = slidelist;

  this.main = this.root.getElementsByTagName("main");
  if (this.main) {
    let linkSection = document.createElement("section");
    linkSection.id = "links";

    const nextLink = this.createLink("next");
    linkSection.append(nextLink);

    const previousLink = this.createLink("previous");
    linkSection.append(previousLink);

    this.main.append(linkSection);
  }


  // <section id="links">
  //     <a id="next" href="slide-7.html" aria-label="next slide"> </a>
  //     <a id="previous" href="slide-5.html" aria-label="previous slide"> </a>
  //   </section>
}

LinkGenerator.prototype.createLink = function ( dir ) {
  // const slide = getSlide(this.slidelist, dir)
  const slide = getSlide(dir)

  let link = document.createElement("a");
  link.setAttribute("href", slide);
  return link;
}
